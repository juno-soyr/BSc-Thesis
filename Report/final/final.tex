...
\begin{abstract}
 In this paper we will focus on the invariance of $\lambda$-calculus with respect to $\beta$-reduction. Accattoli and Dal Lago proposed, using a calculus with explicit substitutions called the \textbf{Linear Substitution Calculus}, a reduction strategy that relates polynomially to Turing Machines when only time complexity is concerned. We will discuss said calculus and strategy and provide an example of how they relate to the classical untyped $\lambda$-calculus and the leftmost outermost strategy, as well as showing that this relation holds for addition using Church Numerals.\end{abstract}
...
\end{center}
The word reasonable here denotes every model equivalent to the (single-tape) Turing machine. What this statement proposes that not only is there a method in which machine models can simulate each other, but that there exists such a method so that the overhead is polynomial in time and constant in space. In other words,if $\lambda$-calculus is a reasonable machine ,it would only take a polynomial amount of extra time and a constant amount of extra space to compute a function $f : \mathbb{N}^k \rightarrow \mathbb{N}$ in it than in a Turing Machine and vice-versa, so every algorithm that could theoretically run in a Turing Machine could be replicated in $\lambda$-calculus.
...
\begin{equation}
  \begin{split}
    & (\lambda mnfx.m f (n f x)) \ulcorner 4 \urcorner \ulcorner 3 \urcorner \rightarrow_{dB} \\
    & (\lambda nfx. m f (n f x )[m \leftarrow \ulcorner 4 \urcorner ]) \ulcorner 3 \urcorner \rightarrow_{dB} \\
    & (\lambda fx. mf (n f x)[m \leftarrow \ulcorner 4 \urcorner])[n \leftarrow \ulcorner 3 \urcorner] \rightarrow_{ls} \\
    & (\lambda fx. \ulcorner 4 \urcorner f (n f x)[\cdots])[n \leftarrow \ulcorner 3 \urcorner] \rightarrow_{ls} \\
    & (\lambda fx. \ulcorner 4 \urcorner f (\ulcorner 3 \urcorner f x)[\cdots])[\cdots] \rightarrow_{dB} \\
    & (\lambda fx.(\lambda x' .f'^4(x')[f'\leftarrow f])(\ulcorner 3 \urcorner f x)) \xrightarrow[ls]{4} \\
    & (\lambda fx. (\lambda x' .f^4(x')) (\ulcorner 3 \urcorner f x)) \rightarrow_{dB} \\
    & (\lambda fx. (f^4(x')[x' \leftarrow \ulcorner 3 \urcorner f x])) \rightarrow_{ls} \\
    & (\lambda fx. (f^4(\ulcorner 3 \urcorner f x))) \rightarrow_{dB} \\
    & (\lambda fx. f^4((\lambda x''.f''^3(x'')[f'' \leftarrow f]x))) \xrightarrow[ls]{3}\\
    & (\lambda fx. f^4(\lambda x''.f^3(x''))x) \rightarrow_{dB} \\
    & (\lambda fx. f^4(f^3(x''))[x'' \leftarrow x])
  \end{split}
\end{equation}
Which, if unfolded, is equal to the church numeral $\ulcorner 7 \urcorner$.
\section{Conclusion}
We have proven that the LSC, and more specifically, LSC under LOU reduction is invariant with respect to $\lambda$-calculus and LO reductions in the context of addition of Church numerals, as well as provide a formal proof for the size-exploding family.
...
Future work could focus on the extrapolation of this invariance result to the more practical area of typed $\lambda$-calculus. This could improve performance of type-checkers, proof assistants and other real-world applications. Another possible avenue is expanding the invariance result in order to remove the explicit substitutions.
