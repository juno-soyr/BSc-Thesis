...
\end{split}
\end{equation*}
While on the untyped $\lambda$-calculus addition takes a constant amount of time, this is due to the fact that the replacement operation has a constant cost, something that is not very practical when talking about complexity measure. We will now show that the implementation of addition in the LSC with LOU reductions takes a linear amount of steps, therefore following the invariance results. 
...
\begin{equation}
	\begin{split}
		...
		& (\lambda fx. \ulcorner a \urcorner f (\urcorner b \ulcorner f x)[\cdots])[\cdots] \rightarrow_{dB} \\
