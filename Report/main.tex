...
\subsection{Leftmost-Outermost Useful}
\label{lou}
The definition of \textit{usefulness} of a substitution step, is defined in base to wether a redex is \textit{useful} in its unfolding of the surrounding (shallow) context.
\paragraph{Useful Step} A reduction step is useful if it is either a $dB$-step or a...
...
We will demonstrate that the LOU reduction strategy follows this generalization by means of -- one, 2? -- examples.
\section{Example 1: Size exploding family}
...
\begin{split}
  ...
  &(\lambda x . yxx)(yxx)[x \leftarrow (yxx)] \rightarrow_{LOU} \\
  &(yzz)[z \rightarrow (yxx)][x \rightarrow (yxx)]
\end{split}
\end{equation}
...
-Talk about the LHR strategy and its difference-
\section{Example 2: Addition and Multiplication}
-- To discuss still --
